<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%><%@ taglib tagdir="/WEB-INF/tags" prefix="s"%><%@ taglib
	uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<s:site><jsp:attribute name="titlePart">
		<title>${tool.descriptor.pluginName}</title>
...
#switcher {
	...
	<div class="screen">
		<div class="detail_head">
			<div class="run_div">
				<input class="btn" type="button" id="run_btn" value="运行"
						onclick="doRun()">
		...
		<div class="input_output">
			...
			<c:forEach var="input" items="${tool.descriptor.inputModel}"
						varStatus="status">
		...
		<c:if test="${tool.status == 0}">
			<div>
				<a
						href="${root}/ctrl/tool/approve?toolId=${tool.descriptor.pluginID}">允许公开</a>
			</div>
		</c:if>
		<div style="clear: left; padding-top: 20px;">
			...
			<div class="left_float"
				...
				<a id="${t.descriptor.pluginID}"
							href="${root}/${t.descriptor.pluginID}.html">
					<c:choose>
						<c:when test="${empty t.descriptor.icon}">
							<div class="tool_logo">
										<span>${t.descriptor.pluginName}</span>
									</div>
						</c:when>
						<c:otherwise>
							<img height="64" width="64" src="${t.descriptor.displayIcon}"
										style="border: 1px #333333 inset;" />
						</c:otherwise>
					</c:choose>
				</a>
			</div>
			</c:forEach>
			<div style="font-size: 12px; clear: left;">
...
<script type="text/javascript" id="bdshare_js"
				data="type=tools&amp;uid=5954609"></script>
		...
		<div class="comments">
			<c:forEach var="c" items="${comments}" varStatus="vs">
				<s:comment comment="${c}" subjectId="${tool.descriptor.pluginID}"
						index="${vs.index + 1}" scope="tool" closable="${canDelComment}"></s:comment>
			</c:forEach>
			<form method="post" action="${root}/ctrl/tool/comment">
				...
				<input type="submit" class="btn" value="评论(字数&lt;5K)"
						onclick="return doSubmit();"><br />
				<input type="hidden" name="toolId"
						value="${tool.descriptor.pluginID}">
			</form>
		</div>
	</div>
</jsp:body></s:site>
